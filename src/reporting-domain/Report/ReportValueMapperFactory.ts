// TODO: some tricky clickHandler stuff here. Need to figure out how to handle this.
// also integrates with store, so need to figure out how to handle that as well.
// TODO: change vuex store to pinia
import { Store } from 'vuex'

import { PaneKind } from '@vuescape/reporting-domain/PaneKind'

import {
  type Guid,
  // TODO: create a type for toEnum and tryToEnum and import
  toEnum,
  tryToEnum,
} from '@vuescape/types'
...
import {
  ...
  UiObjectType,
  UnitOfMeasure,
} from '@vuescape/reporting-domain'
...
export class ReportValueMapperFactory {
  ...
  private processRow(
    ...
    if (row.children && row.children.length > 0) {
      row.children.forEach((child: TreeTableRow) => this.processRow(child, behaviors, clickHandler))
    }

    if (row.expandedSummaryRows && row.expandedSummaryRows.length > 0) {
      row.expandedSummaryRows.forEach((expandedSummaryRow: TreeTableRow) => this.processRow(
        expandedSummaryRow,
        behaviors,
        clickHandler,
      ))
    ...
    if (row.collapsedSummaryRows && row.collapsedSummaryRows.length > 0) {
      row.collapsedSummaryRows.forEach((collapsedSummaryRow: TreeTableRow) => this.processRow(
        collapsedSummaryRow,
        behaviors,
        clickHandler,
      ))
